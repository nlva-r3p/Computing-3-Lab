...
class Salaried : public PayKind { // derived class
	public:
		...
		// lab 9
		double payAdvice() override {
			return _annualSalary/26;
		}
...
class PerCourse : public PayKind {
	public:
		...
		double payAdvice() override {
			return _feePerCourse/7;
		}

		// accessor functions
		unsigned getPerCourseFee() const { return _feePerCourse; }

		void output(ostream& out) const { out << *this; }

	friend ostream& operator<< (ostream&, const PerCourse&);

	void setFeePerCourse(unsigned feePerCourse) { _feePerCourse = feePerCourse; }
...
class PartTime : public EmployeeKind {
	public:
		...
		// accessor functions
		unsigned getPartTimeHours() const { return _maxHours; }

		void output(ostream& out) const { out << *this; }

	friend ostream& operator<< (ostream&, const PartTime&);

	void setMaxHours(unsigned maxHours) { _maxHours = maxHours; }
...
ostream& operator<< (ostream& out, const PartTime& s) {
	out << "Max Hours:\t" << s.getPartTimeHours() << endl;

    return out;
}

ostream& operator<< (ostream& out, const PerCourse& s) {
	out << "Per Course Fee:\t" << s.getPerCourseFee() << endl;
...
class Adjunct : public Faculty, public PerCourse, public PartTime {
	public:
		...
		Adjunct(string firstName, string lastName, string address, unsigned _IDNumber,
			string SSN, string department, string rank, unsigned feePerCourse, 
			unsigned maxHours, unsigned maxCourses);
		...
		// mutator functions
		void setMaxCourses(unsigned maxCourses) { _maxCourses = maxCourses; }
...
Adjunct::Adjunct(string firstName, string lastName, string address, unsigned _IDNumber,
			string SSN, string department, string rank, unsigned feePerCourse, 
			unsigned maxHours, unsigned maxCourses)
			: Faculty(firstName, lastName, address, _IDNumber, SSN, department), PerCourse(feePerCourse), PartTime(maxHours)
			{
...
ostream& operator<< (ostream& out, const Adjunct& n) {
	...
	n.PerCourse::output(out);

	n.PartTime::output(out);
...
int main(void) {
    ...
    Grader g1("John", "Doe", "89 Canal Street", 54329876, 3.5, "CS", "Tom Wilkes", "junior",
        "COMP.2010", 25.00, 30);
    cout << "Grader g1(John...):\n" << g1;
    cout << "Grader g1's Pay Advice: " << g1.payAdvice() << '\n' << endl;

    Grader g2(g1); // invoke copy constructor
    cout << "\nGrader g2(copy of g1 John):\n" << g2;
    cout << "Grader g2's Pay Advice: " << g2.payAdvice() << '\n' << endl;
    ...
    cout << "NTT n1's Pay Advice: " << n1.payAdvice() << '\n' << endl;

    Adjunct adjunctProfessor;
    adjunctProfessor.setMaxCourses(3);
    adjunctProfessor.setMaxHours(20);
    adjunctProfessor.setFeePerCourse(1500);

    cout << "Adjunct Professor Information:\n" << adjunctProfessor;
    cout << "Adjunct's Pay Advice: " << adjunctProfessor.payAdvice() << '\n' << endl;
	...
	/*
	1) For the print output function, we will declare it as a virtual const function, such that the derived class's implementation of the output function
	will override the base function's output function. This will allow the for loop to select which class's implementation of output function to use
	depending on the dereferenced object. E.g. if the deferenced object is of derived class NTT, then it will use the output function in NTT, who would then call
	the overloaded operator<< in NTT to output the info of NTT. Because we didn't use virtual functions before, it will default to using the functions defined in the base
	class. Thus, it will call the overloaded operator<< in the base class, which would only print out the UMLPerson's member variables. While using virtual functions would
	utilize runtime polymorphism to determine which class's output function to call based on its object.
	
	2) The program would not run because Pay Advice is a member function of the PayKind class and its descendents. Thus, since we are not utilizing a virtual function implementation,
	the function would not be called via polymorphism because it does not exist in the base class. A way to fix this would to declare PayAdvice as a virtual function 
	in the base class. However, that would mess things up knowing that it is a function exclusively for PayKind and its descendents, so we can adjust the override output function
	for classes that utilize PayKind and its descendents to include the output for PayAdvice.
	*/
